...
msgLenToPlot = [u'64', u'512', u'1024']
...
# path = "/tmp/"
resultsFile = "./diss-64_512_1024_delay_2by3_Fl_07-5runs-noLoadWithLoad.pdf"
...
def filterData(messageLengths, offsets, delays,
    ...
    print "sentValuesDict:"
    print sentValuesDict
    print "receivedValuesDict"
    print receivedValuesDict
                        ...
                        if delay not in delayToPlot:
                            ...
                            for frameLen in frameLengths:
                                if frameLen not in frameLenToPlot:
                                    print key, msgLen, offset, delay, frameLen
...
def plot(noLoadData, withLoadData):
    ...
    plt.ylabel('Accuracy (%)')
    c = 0
    ax.text(c+3.1, 110.0, u'M=64')
    ax.text(c+11.1, 110.0, u'M=512')
    ax.text(c+19.1, 110.0, u'M=1024')
...
def main():
    ...
    withLoadlevenshteinDict = getLevenshtein(messageLengths, offsets, delays, frameLengths, timeIntervals, numberOfRuns,
                                     sentValuesDict["messageDict"], receivedValuesDict["messageDict"])
